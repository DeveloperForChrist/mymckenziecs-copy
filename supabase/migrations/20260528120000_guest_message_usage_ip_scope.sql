-- Enforce anonymous chat cooldowns by both browser guest id and client IP key.
-- The IP is hashed by the app into a deterministic UUID before this function is called.
...
BEGIN
  ...
  SET
    message_count = CASE
      WHEN window_start IS NULL OR (now_ts - window_start) >= window_interval THEN 1
      ELSE message_count + 1
    END,
    window_start = CASE
      WHEN window_start IS NULL OR (now_ts - window_start) >= window_interval THEN now_ts
      ELSE window_start
    END,
    updated_at = now_ts
  WHERE guest_id = ANY(usage_ids);

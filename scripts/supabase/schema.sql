...
CREATE TABLE admin_metric_rollups (
...
CREATE INDEX idx_subscriptions_scheduled_change ON subscriptions(scheduled_change_at DESC) WHERE scheduled_plan_type IS NOT NULL;
CREATE INDEX idx_chat_uploads_owner_expires ON chat_uploads(owner_id, expires_at DESC);
CREATE INDEX idx_chat_uploads_expires ON chat_uploads(expires_at DESC);
CREATE INDEX idx_user_entitlements_paid_access ON user_entitlements(paid_access, updated_at DESC);
